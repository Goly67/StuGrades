let subjects = [];
        ...
        function updateGrade(index, grade) {
            grade = grade.replace(/[^0-9]/g, '').slice(0, 2);
            if (grade) {
        ...
        function renderTable() {
            ...
            subjects.forEach((subject, index) => {
                ...
                gradeCell.innerHTML = `<input type="text" inputmode="numeric" pattern="[0-9]*" maxlength="2" class="grade-input" value="${currentGrade}" onchange="updateGrade(${index}, this.value)">`;
        ...
        function calculateAverage() {
            ...
            // Loop through each subject and add the grades to total and count
            subjects.forEach(subject => {
                if (subject.firstQuarter !== null && !isNaN(subject.firstQuarter)) {
                    total += subject.firstQuarter;
                    count++;
                }
                if (subject.secondQuarter !== null && !isNaN(subject.secondQuarter)) {
                    total += subject.secondQuarter;
                    count++;
                }
                if (subject.thirdQuarter !== null && !isNaN(subject.thirdQuarter)) {
                    total += subject.thirdQuarter;
                    count++;
                }
                if (subject.fourthQuarter !== null && !isNaN(subject.fourthQuarter)) {
                    total += subject.fourthQuarter;
                    count++;
                }
            });

            // If no valid grades exist, show message
            if (count === 0) {
            ...
            if (average >= 98) {
                averageDisplay += ' - With Highest Honors';
                averageClass = 'with-highest-honors';
            } else if (average >= 95) {
                averageDisplay += ' - With High Honors';
                averageClass = 'with-high-honors';
            } else if (average >= 90) {
                averageDisplay += ' - With Honors';
                averageClass = 'with-honors';
            }
        ...
        document.getElementById('quarter-select').addEventListener('change', function (e) {
